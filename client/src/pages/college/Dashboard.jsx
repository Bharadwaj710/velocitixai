...
const CollegeDashboard = () => {
  ...
  const hiredStudents = studentsData.filter(
    (student) => student.hired && student.company
  );

  // Get unique domains and skills from studentsData
  const allDomains = Array.from(
    new Set(studentsData.map((s) => s.domain).filter(Boolean))
  );
  const allSkills = Array.from(
    new Set(studentsData.flatMap((s) => s.skills || []).filter(Boolean))
  );
  ...
  const filteredStudents = useMemo(() => {
    return studentsData.filter((student) => {
      const matchesDomain =
        selectedDomain === "all" || student.domain === selectedDomain;
      const matchesSkill =
        selectedSkill === "all" ||
        (student.skills && student.skills.includes(selectedSkill));
      const matchesSearch =
        ...
        (student.coursePath?.toLowerCase() || "").includes(
          searchTerm.toLowerCase()
        );
                ...
                <div className="absolute right-0 mt-2 w-56 bg-white border rounded-lg shadow-lg py-2 z-50">
                  <div className="px-4 py-3 border-b border-gray-100">
                    ...
                    <div className="flex items-center text-xs text-gray-500 mb-1">
                      {user?.phoneNumber && <span className="mr-1">📞</span>}
                      {user?.phoneNumber || "No phone"}
                    </div>
                    <div className="flex items-center text-xs text-gray-500">
                      {user?.address && <span className="mr-1">📍</span>}
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-6">
                {filteredStudents.map((student) => (
                  <div
                    ...
                    className="bg-white rounded-xl p-6 shadow hover:shadow-lg transition flex flex-col items-center border border-gray-100 min-w-0"
                  ...
                  >
                    ...
                    <h3 className="font-semibold text-lg text-gray-900 text-center mb-1">
                    ...
                    <div className="w-full bg-gray-200 rounded-full h-2 mb-2">
                      <div
                        className={`h-2 rounded-full ${
                          allCoursesProgress[student._id || student.id] > 0
                            ? "bg-green-400"
                            : "bg-gray-300"
                        }`}
                        style={{
                          ...
                          const progressArr = await Promise.all(
                            student.course.map(async (c) => {
                              try {
                                ...
                                const completed =
                                  res.data.completedLessons?.length || 0;
                                return {
          ...
          <div className="bg-white rounded-lg shadow-xl p-8 max-w-lg w-full relative">
            ...
            <div className="space-y-2">
              <div>
                ...
                {selectedStudent.user?.name || selectedStudent.name || "N/A"}
              </div>
              <div>
              ...
              <div>
                <span className="font-semibold">College Course:</span>{" "}
                {selectedStudent.collegecourse || "N/A"}
              </div>
              <div>
                ...
                selectedStudent.course.length > 0 ? (
                  <ul className="list-disc ml-6">
                    {selectedStudent.course.map((c, idx) => (
                      <li key={idx}>
                        {c?.title || c?.name || c?._id || "Unknown Course"}
                      </li>
                    ))}
                  </ul>
                ) : (
              ...
              <div>
                ...
                {selectedStudent.hired?.isHired ? "Yes" : "No"}
              </div>
              {selectedStudent.hired?.isHired && (
                <>
                  <div>
                    <span className="font-semibold">Company Name:</span>{" "}
                    {selectedStudent.hired.companyName || "N/A"}
                  </div>
                  <div>
                    <span className="font-semibold">Hired Date:</span>{" "}
                    {selectedStudent.hired.hiredDate
                      ? new Date(
                          selectedStudent.hired.hiredDate
                        ).toLocaleDateString()
                      : "N/A"}
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-40">
          <div className="bg-white rounded-lg shadow-xl p-8 max-w-lg w-full relative">
            <button
              className="absolute top-2 right-2 text-gray-400 hover:text-gray-600 text-xl"
              onClick={() => {
                setStudentProgressModalOpen(false);
                setStudentProgress([]);
              }}
              aria-label="Close"
            >
                ...
                {studentProgress.map((p, idx) => (
                  <li key={idx}>
                    ...
                    <div className="text-xs text-gray-500 mt-1">
                      {p.completed} of {p.totalLessons} lessons completed
                    </div>
                  </li>
                ))}
              </ul>
